...
def load_datasets_and_vocabs(args):
    ...
    # 我们的模型采用展开数据，目前我们不考虑MAMS的情况(未来的实验)
    # 将多方面的句子展开成单方面的 句子
    ...
    # 构建单词词汇表(part of speech\词性, dep_tag\依存标签)并保存pickle。
...
def get_rolled_and_unrolled_data(input_data, args):
    ...
    在input_data中，每个句子可以包含多个方面，具有不同的情感。
    我们的方法每次用一个方面来处理每个句子，所以即使对于
    多方面多情感句子，将其展开为单方面句子。
    对带有aspect的每个句子执行reshape_dependency_tree
    input：
        input_data: 读取的json格式的数据
    ...
    for e in input_data:
        e['tokens'] = [x.lower() for x in e['tokens']] # 单词最小化
        aspects = []
        sentiments = []
        froms = []
        tos = []
        ...
        for i in range(len(e['aspect_sentiment'])):
            ...
            # We would tokenize the aspect while at it. 对方面进行分词
            ...
            # Center on the aspect. 集中在方面术语上  重塑造基于方面的依赖树
